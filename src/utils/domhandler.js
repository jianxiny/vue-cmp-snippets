export default {
  ...
  /**
   * Determines if the current environment is a client-side environment.
   *
   * @return {boolean} Returns true if running on client-side, otherwise false.
   */
  isClient() {
  ...
  getNextFocusableElement(container, element, selector) {
    ...
    const index =
      focusableElements.length > 0 ? focusableElements.findIndex((el) => el === element) : -1
